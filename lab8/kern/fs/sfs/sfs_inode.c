...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
  ...
  //LAB8:EXERCISE1 2010011324 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	/*
...
sfs_unlink_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, const char *name)
{
    ...
    if (lnksin->din->type != SFS_TYPE_DIR) {
        ...
        cprintf("Unlink directory not implemented!\n");
        ret = -E_UNIMP;
    }
    vop_ref_dec(link_node);
    return ret;
